...
// ListFeed returns the provider's recent notifications.
...
// RealtimeToken mints a short-lived websocket token for the provider.
...
// RegisterDevice stores the provider's push device token.
