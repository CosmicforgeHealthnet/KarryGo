...
| Go module | `cosmicforge/logistics/services/dispatch-delivery-service` |
...
| Docker image | Multi-stage Go build; Alpine 3.22 runtime; non-root `cosmicforge` user |
| Readiness | `/ready` pings PostgreSQL and Redis |
| Current Compose storage | VPS/local mounted volume `../tmp/cosmicforge-uploads:/app/uploads`; `VERIFICATION_STORAGE_MODE=local_private` |
...
Compose-only PostgreSQL container variables are `POSTGRES_DB=dispatch_delivery_service`, `POSTGRES_USER=cosmicforge_logistics`, and `POSTGRES_PASSWORD=cosmicforge_logistics`; the Go service does not read them directly. `FIREBASE_PROJECT_ID` and `FIREBASE_CREDENTIALS_PATH` appear only as commented placeholders in `.env.example` and are not read by production code. Asynq concurrency and queue weights are hardcoded in `cmd/main.go`; no Asynq-specific environment variables were found.
