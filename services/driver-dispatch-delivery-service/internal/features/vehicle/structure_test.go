...
func TestIsValidBikeType(t *testing.T) {
	valid := []BikeType{BikeMotorcycle, BikeDispatch, BikeScooter, BikeTricycle, BikeBicycle, BikeElectric}
	for _, bt := range valid {
		if !IsValidBikeType(bt) {
			t.Fatalf("IsValidBikeType(%q) = false, want true", bt)
		}
	}
