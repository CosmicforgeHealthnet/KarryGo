...
func (s *Service) UpdateBike(ctx context.Context, bikeID, providerID, correlationID string, input UpdateBikeInput) (Bike, error) {
	...
	// Block updates on verified or suspended bikes.
	if current.VerificationStatus == VehicleVerified || current.VerificationStatus == VehicleSuspended {
		return Bike{}, apperrors.Conflict("Bike details cannot be updated in its current status.", nil)
	}
...
func validateRegisterInput(input RegisterBikeInput) error {
	...
	if !IsValidBikeType(input.BikeType) {
		...
		if input.BikeType != "" {
			msg = "Bike type must be motorcycle or dispatch_bike."
		}
...
func hasAnyUpdateField(input UpdateBikeInput) bool {
	return input.Brand != nil || input.Model != nil || input.Year != nil || input.Color != nil ||
		input.EngineCc != nil || input.ChassisNumber != nil
}
