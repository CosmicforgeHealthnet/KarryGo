...
func TestUpdateBikePlateNumberSucceeds(t *testing.T) {
	repo := newFakeRepository()
	providerID := "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"
	...
	if data["plate_number"] != "NEW-PLATE-XX" {
		t.Fatalf("plate_number = %v, want NEW-PLATE-XX", data["plate_number"])
	}
	if data["color"] != "Yellow" {
		t.Fatalf("color = %v, want Yellow", data["color"])
	}
}

func TestUpdateBikeTypeSucceeds(t *testing.T) {
	repo := newFakeRepository()
	providerID := "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"
	bike := repo.seedBike(providerID, "LAG-TYPE-01", true)
	if bike.BikeType == BikeDispatch {
		t.Skip("seed bike already has dispatch_bike; skip to avoid false positive")
	}
	...
	data := extractVehicleData(t, w.Body.Bytes())
	if data["bike_type"] != "dispatch_bike" {
		t.Fatalf("bike_type = %v, want dispatch_bike", data["bike_type"])
	}
}

func TestUpdateVerifiedBikeSucceeds(t *testing.T) {
	repo := newFakeRepository()
	providerID := "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"
	...
	if data["color"] != "Silver" {
		t.Fatalf("color = %v, want Silver", data["color"])
	}
}
...
func TestUpdateUnverifiedBikePublishesVehicleRegisteredEvent(t *testing.T) {
	repo := newFakeRepository()
	providerID := "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"
	bike := repo.seedBike(providerID, "LAG-EVU-01", true) // seedBike starts as unverified
	...
	if ep.registeredCount != 1 {
		t.Fatalf("PublishVehicleRegistered called %d times after PATCH on unverified bike, want 1", ep.registeredCount)
	}
}

func TestUpdateVerifiedBikeWithoutIdentityChangeDoesNotPublishRegisteredEvent(t *testing.T) {
	repo := newFakeRepository()
	providerID := "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"
	bike := repo.seedBikeWithStatus(providerID, "LAG-EVV-01", true, VehicleVerified)
	...
	body := `{"color":"Silver"}` // color-only change on verified bike
	req := httptest.NewRequest(http.MethodPatch, "/api/v1/provider/vehicle/"+bike.ID, strings.NewReader(body))
	req.Header.Set("Authorization", "Bearer "+token)
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	...
	if ep.registeredCount != 0 {
		t.Fatalf("PublishVehicleRegistered called %d times for color-only change on verified bike, want 0", ep.registeredCount)
	}
}
...
func TestAdminReviewMissingActionReturns400(t *testing.T) {
	...
	router, tokens := buildVehicleTestRouter(repo)
	token := mustVehicleRoleToken(t, tokens, "66666666-6666-6666-6666-666666666666", "platform_admin")
...
func TestAdminApproveUpdatesBikeStatusToVerified(t *testing.T) {
	...
	svc := NewService(repo, WithEventPublisher(ep))
	router, tokens := buildVehicleTestRouterWithSvc(svc)
	token := mustVehicleRoleToken(t, tokens, "77777777-7777-7777-7777-777777777777", "platform_admin")
...
func TestVehicleVerifiedEventHasProviderIDAndBikeID(t *testing.T) {
	...
	ep := &fakeEventPublisher{}
	svc := NewService(repo, WithEventPublisher(ep))
	_, err := svc.AdminReview(context.Background(), bikeID, "66666666-6666-6666-6666-666666666666", "corr-vev",
...
func TestVehicleRejectedEventHasReason(t *testing.T) {
	...
	ep := &fakeEventPublisher{}
	svc := NewService(repo, WithEventPublisher(ep))
	_, err := svc.AdminReview(context.Background(), bikeID, "66666666-6666-6666-6666-666666666666", "corr-rej",
...
func TestVehicleSuspendedEventHasReason(t *testing.T) {
	...
	ep := &fakeEventPublisher{}
	svc := NewService(repo, WithEventPublisher(ep))
	_, err := svc.AdminReview(context.Background(), bikeID, "66666666-6666-6666-6666-666666666666", "corr-sus",
...
func mustVehicleRoleToken(t *testing.T, tokens *authusecases.TokenUsecase, providerID, role string) string {
	t.Helper()
	token, _, err := tokens.GenerateAccessToken(providerID, "+2348000000001", "33333333-3333-3333-3333-333333333333")
	if err != nil {
...
func (r *fakeRepository) UpdateBike(_ context.Context, bikeID, providerID string, input UpdateBikeInput) (Bike, error) {
	for i, b := range r.bikes {
		if b.ID == bikeID && b.ProviderID == providerID {
			...
			if input.BikeType != nil {
				r.bikes[i].BikeType = *input.BikeType
			}
			if input.PlateNumber != nil {
				r.bikes[i].PlateNumber = *input.PlateNumber
			}
