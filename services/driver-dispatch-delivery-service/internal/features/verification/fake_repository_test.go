...
func (r *fakeVerificationRepository) UpdateLicenceExpiry(_ context.Context, providerID string, expiryDate time.Time) error {
	if r.err != nil {
		return r.err
	}
	if steps, ok := r.steps[providerID]; ok {
		if step, ok := steps[StepLicence]; ok {
			step.LicenceExpiryDate = &expiryDate
			steps[StepLicence] = step
		}
	}
	return nil
}
