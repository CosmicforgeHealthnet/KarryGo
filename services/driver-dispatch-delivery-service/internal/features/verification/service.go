...
func (s *Service) SubmitLicence(ctx context.Context, input LicenceSubmissionInput) (StepSubmissionResponse, error) {
	...
	// Persist the licence expiry date (last day of the given month/year).
	if year, convErr := strconv.Atoi(input.ExpiryYear); convErr == nil {
		if month, convErr := strconv.Atoi(input.ExpiryMonth); convErr == nil {
			// Last day of the expiry month: first day of next month minus one day.
			expiryDate := time.Date(year, time.Month(month+1), 1, 0, 0, 0, 0, time.UTC).AddDate(0, 0, -1)
			if persistErr := s.repository.UpdateLicenceExpiry(ctx, input.ProviderID, expiryDate); persistErr != nil {
				log.Printf("verification: SubmitLicence: failed to persist expiry for provider_id=%s: %v", input.ProviderID, persistErr)
			}
		}
	}
...
func (s *Service) ApplyVehicleRegistered(ctx context.Context, event VehicleRegisteredEvent) error {
	if err := validateProviderID(event.ProviderID); err != nil {
		return err
	}
	...
	if !ok {
		log.Printf("verification vehicle.registered provider_id=%s ignored because vehicle step does not exist", event.ProviderID)
		return nil
	}
	if previous.Status == StatusSubmitted {
		log.Printf("verification vehicle.registered provider_id=%s ignored because vehicle step is already submitted", event.ProviderID)
		return nil
	}

	updated, err := s.repository.UpdateStepSubmitted(ctx, event.ProviderID, StepVehicle)
	if err != nil {
		return err
	}

	notes := "Vehicle registered — step marked as submitted."
...
func summarizeSteps(steps []VerificationStep) []VerificationStepSummary {
	...
	for _, step := range steps {
		result = append(result, VerificationStepSummary{
			...
			LicenceExpiryDate: step.LicenceExpiryDate,
		})
	}
	return result
}
