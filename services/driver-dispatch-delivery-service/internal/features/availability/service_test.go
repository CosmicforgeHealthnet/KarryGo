...
func assertGateError(t *testing.T, err error, code string) {
	...
	reason, ok := appErr.Details["gate"].(GateError)
	if !ok {
		t.Fatalf("details[gate] = %#v, want GateError", appErr.Details["gate"])
	}
	if reason.Code != code {
...
type fakeAvailabilityRepository struct {
	...
	licenceExpiry       map[string]*time.Time
...
func (r *fakeAvailabilityRepository) GetLicenceExpiryDate(_ context.Context, providerID string) (*time.Time, error) {
	return r.licenceExpiry[providerID], nil
}
