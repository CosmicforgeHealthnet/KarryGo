...
// CreateWithEmail creates a login OTP that also carries an email address so the
// Verify step can attach it to the upserted identity without a second DB lookup.
func (u *OTPUsecase) CreateWithEmail(ctx context.Context, phoneNumber, email string) (authmodels.OTP, string, error) {
	if err := u.checkLimit(ctx, phoneNumber); err != nil {
		return authmodels.OTP{}, "", err
	}
	code, err := u.GenerateCode()
...
// CreateForPhoneChange creates an OTP for the phone_change flow keyed by the
// new phone being verified. It reuses that phone's rate-limit bucket so callers
// cannot bypass the global OTP rate limit.
func (u *OTPUsecase) CreateForPhoneChange(ctx context.Context, phoneNumber string) (authmodels.OTP, string, error) {
	if err := u.checkLimit(ctx, phoneNumber); err != nil {
		return authmodels.OTP{}, "", err
	}
	code, err := u.GenerateCode()
...
// VerifyLatestForPurpose verifies the most-recent OTP for the given phone that
// matches a specific purpose. This prevents a "phone_change" OTP from being
// consumed by the login path (and vice-versa).
func (u *OTPUsecase) VerifyLatestForPurpose(ctx context.Context, phoneNumber, purpose, code string) (authmodels.OTP, error) {
	if err := authmodels.ValidateOTPCode(code); err != nil {
		return authmodels.OTP{}, err
	}

	repo, ok := u.repository.(interface {
		LatestByPhoneAndPurpose(ctx context.Context, phone, purpose string) (authmodels.OTP, bool, error)
	})
	if !ok {
		return authmodels.OTP{}, apperrors.Internal("OTP repository does not support purpose-scoped lookup.", nil)
	}

	otp, found, err := repo.LatestByPhoneAndPurpose(ctx, phoneNumber, purpose)
	if err != nil {
		return authmodels.OTP{}, err
	}
	if !found {
