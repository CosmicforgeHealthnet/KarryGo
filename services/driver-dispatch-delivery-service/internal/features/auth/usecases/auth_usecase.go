...
type Options struct {
	...
	Notifier   authclients.NotificationClient
	// EmailClient delivers OTP codes via email alongside SMS.
	// Nil or NoopEmailClient disables email delivery (SMS-only mode).
	EmailClient authclients.EmailClient
	Publisher   authclients.EventPublisher
...
type StartInput struct {
	PhoneNumber   string
	// Email is optional. When non-empty, the same OTP code is also sent to this
	// address via fire-and-forget. Email failure never blocks SMS delivery.
	Email         string
...
//
// Email delivery rules (additive — SMS is always the primary channel):
//  1. If caller provides an email, normalize it and validate it is not already
//     owned by a different phone number.  If valid, persist it on the identity
//     (if one exists for this phone) and use it as the email target.
//  2. If no email is provided (or the provided email was a duplicate), fall back
//     to the email stored on the existing identity for this phone, if any.
//  3. After SMS succeeds, fire-and-forget the same OTP code to the email target.
//     Email failure is logged but never blocks the SMS response.
func (u *AuthUsecase) Start(ctx context.Context, input StartInput) (StartResult, error) {
	...
	if input.Email != "" {
		normalEmail, normalErr := NormalizeEmail(input.Email)
		if normalErr == nil {
			if u.identities != nil {
				// Duplicate-email safety check: skip if email belongs to another phone.
				existingByEmail, emailExists, _ := u.identities.FindByEmail(ctx, normalEmail)
				if emailExists && existingByEmail.PhoneNumber != phoneNumber {
					log.Printf("dispatch rider start: email already registered to another account, skipping email delivery")
				} else {
					emailTarget = normalEmail
					// Persist email on an existing identity when it is new or changed.
					identity, identityExists, _ := u.identities.FindByPhone(ctx, phoneNumber)
					if identityExists && (identity.Email == nil || *identity.Email != normalEmail) {
						if updateErr := u.identities.UpdateEmail(ctx, identity.ID, normalEmail); updateErr != nil {
							log.Printf("dispatch rider start: could not persist email on identity id=%s err=%v", identity.ID, updateErr)
						}
					}
				}
			} else {
				// No identity repository available — accept the email as-is (dev / test mode).
				emailTarget = normalEmail
			}
		}
	}

	// Fall back to the email stored on the existing identity for this phone.
	if emailTarget == "" && u.identities != nil {
		identity, exists, _ := u.identities.FindByPhone(ctx, phoneNumber)
		if exists && identity.Email != nil {
			emailTarget = *identity.Email
		}
	}

	// ── Create OTP ───────────────────────────────────────────────────────────

	// Store the email on the OTP row so Verify can attach it to the identity
	// even when the identity does not exist yet at Start time.
	var otp authmodels.OTP
	var code string
	if emailTarget != "" {
		otp, code, err = u.otp.CreateWithEmail(ctx, phoneNumber, emailTarget)
	} else {
	...
	// ── Fire-and-forget email (secondary channel) ─────────────────────────────

	if u.emailClient != nil && emailTarget != "" {
		emailCopy, codeCopy := emailTarget, code
		go func() {
			if sendErr := u.emailClient.SendOTP(context.Background(), emailCopy, codeCopy); sendErr != nil {
				log.Printf("dispatch rider otp email send failed email=%s err=%v", emailCopy, sendErr)
			}
		}()
	}
...
func (u *AuthUsecase) SignupStart(ctx context.Context, input SignupStartInput) (StartResult, error) {
	...
	// Fire-and-forget email delivery. Never blocks SMS success path.
	if u.emailClient != nil && email != "" {
		emailTarget, otpCode := email, code
		go func() {
			if sendErr := u.emailClient.SendOTP(context.Background(), emailTarget, otpCode); sendErr != nil {
				log.Printf("dispatch rider signup otp email send failed email=%s err=%v", emailTarget, sendErr)
			}
		}()
	}
...
func (u *AuthUsecase) LoginStart(ctx context.Context, input LoginStartInput) (StartResult, error) {
	...
	// Fire-and-forget email delivery to the stored identity email (if any).
	// Never blocks SMS success path.
	if u.emailClient != nil && identityEmail != nil {
		emailTarget, otpCode := *identityEmail, code
		go func() {
			if sendErr := u.emailClient.SendOTP(context.Background(), emailTarget, otpCode); sendErr != nil {
				log.Printf("dispatch rider login otp email send failed email=%s err=%v", emailTarget, sendErr)
			}
		}()
	}
...
func (u *AuthUsecase) Verify(ctx context.Context, input VerifyInput) (TokenResult, error) {
	...
	default:
		...
		// Attach OTP email to the identity when the identity has none.
		// The OTP row carries the email that was supplied (or looked up) during Start.
		if verifiedOTP.Email != nil && identity.Email == nil {
			if emailErr := u.identities.UpdateEmail(ctx, identity.ID, *verifiedOTP.Email); emailErr != nil {
				log.Printf("dispatch rider verify: could not attach otp email to identity id=%s err=%v", identity.ID, emailErr)
			} else {
				identity.Email = verifiedOTP.Email
			}
		}
	}
...
type PhoneChangeStartInput struct {
	// ProviderID is extracted from the validated JWT (not from the request body).
	ProviderID    string
	NewPhone      string
	CorrelationID string
}

type PhoneChangeVerifyInput struct {
	ProviderID    string
	NewPhone      string
	OTPCode       string
	CorrelationID string
...
// PhoneChangeStart sends an OTP to the provider's new phone number.
func (u *AuthUsecase) PhoneChangeStart(ctx context.Context, input PhoneChangeStartInput) (StartResult, error) {
	identity, exists, err := u.identities.GetByID(ctx, input.ProviderID)
	if err != nil {
		return StartResult{}, err
	}
	if !exists {
		return StartResult{}, apperrors.NotFound("Account not found.", nil)
	}

	currentPhone := identity.PhoneNumber
	newPhone, err := NormalizePhoneNumber(input.NewPhone)
	if err != nil {
		return StartResult{}, apperrors.Validation("Check your details.", []apperrors.FieldViolation{
			{Field: "new_phone_number", Message: "Phone number must be a valid E.164 number."},
		})
	}
	if currentPhone == newPhone {
		return StartResult{}, apperrors.BadRequest("New phone number must differ from the current phone number.", nil)
	}

	otherIdentity, alreadyExists, err := u.identities.FindByPhone(ctx, newPhone)
	if err != nil {
		return StartResult{}, err
	}
	if alreadyExists && otherIdentity.ID != input.ProviderID {
		return StartResult{}, apperrors.Conflict("An account with this phone number already exists.", nil)
	}

	_, code, err := u.otp.CreateForPhoneChange(ctx, newPhone)
	if err != nil {
		return StartResult{}, err
	}

	if u.notifier != nil {
		if err := u.notifier.SendOTP(ctx, newPhone, code); err != nil {
			return StartResult{}, apperrors.Unavailable("Verification code delivery is temporarily unavailable.", err)
		}
	}

	if u.otpDebug {
		log.Printf("development dispatch rider phone_change otp phone_number=%s otp=%s", newPhone, code)
	}

	return StartResult{ExpiresInSeconds: u.otp.TTLSeconds()}, nil
}

// PhoneChangeVerify validates the OTP that was sent to the provider's new
// phone, then atomically updates the phone in both dispatch_rider_identities and
// providers. It returns an error if new_phone is already registered.
func (u *AuthUsecase) PhoneChangeVerify(ctx context.Context, input PhoneChangeVerifyInput) (PhoneChangeResult, error) {
	identity, exists, err := u.identities.GetByID(ctx, input.ProviderID)
	if err != nil {
		return PhoneChangeResult{}, err
	}
	if !exists {
		return PhoneChangeResult{}, apperrors.NotFound("Account not found.", nil)
	}
	currentPhone := identity.PhoneNumber
	newPhone, err := NormalizePhoneNumber(input.NewPhone)
	if err != nil {
		return PhoneChangeResult{}, apperrors.Validation("Check your details.", []apperrors.FieldViolation{
			{Field: "new_phone_number", Message: "Phone number must be a valid E.164 number."},
		})
	}
	if currentPhone == newPhone {
		return PhoneChangeResult{}, apperrors.BadRequest("New phone number must differ from the current phone number.", nil)
	}

	// Reject if new phone already registered.
	otherIdentity, alreadyExists, err := u.identities.FindByPhone(ctx, newPhone)
	if err != nil {
		return PhoneChangeResult{}, err
	}
	if alreadyExists && otherIdentity.ID != input.ProviderID {
		return PhoneChangeResult{}, apperrors.Conflict("An account with this phone number already exists.", nil)
	}

	// Verify the OTP that was sent to the new phone for purpose=phone_change.
	if _, err := u.otp.VerifyLatestForPurpose(ctx, newPhone, "phone_change", input.OTPCode); err != nil {
		return PhoneChangeResult{}, err
	}

	if err := u.identities.UpdatePhone(ctx, input.ProviderID, currentPhone, newPhone); err != nil {
		return PhoneChangeResult{}, err
	}

	if u.publisher != nil {
		event := authclients.PhoneChangedEvent{
			Event:         authclients.TopicPhoneChanged,
			CorrelationID: input.CorrelationID,
		...
		if err := u.publisher.PublishPhoneChanged(ctx, event); err != nil {
			return PhoneChangeResult{}, apperrors.Internal("Phone change event publishing failed.", err)
		}
	}

	return PhoneChangeResult{Message: "Phone number updated successfully.", PhoneNumber: newPhone}, nil
}
