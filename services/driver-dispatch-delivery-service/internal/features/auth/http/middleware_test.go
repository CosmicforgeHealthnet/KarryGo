...
func TestDispatchRiderAuthRequiredAllowsValidAccessTokenAndAttachesContext(t *testing.T) {
	...
	rawToken, _, err := tokens.GenerateAccessToken(riderID, "+2348012345678", sessionID)
	if err != nil {
	...
	if data["dispatch_rider_id"] != riderID {
		t.Fatalf("dispatch_rider_id = %v, want %s", data["dispatch_rider_id"], riderID)
	}
	if data["phone_number"] != "+2348012345678" {
		t.Fatalf("phone_number = %v, want +2348012345678", data["phone_number"])
	}
	if data["session_id"] != sessionID {
		t.Fatalf("session_id = %v, want %s", data["session_id"], sessionID)
	}
...
func TestDispatchRiderAuthRequiredRejectsExpiredAccessToken(t *testing.T) {
	...
	rawToken, _, err := tokens.GenerateAccessToken(uuid.NewString(), "+2348012345678", uuid.NewString())
	if err != nil {
		t.Fatalf("GenerateAccessToken() error = %v", err)
	}
...
func TestDispatchRiderAuthRequiredRejectsWrongSignature(t *testing.T) {
	...
	rawToken, _, err := signingTokens.GenerateAccessToken(uuid.NewString(), "+2348012345678", uuid.NewString())
	if err != nil {
		t.Fatalf("GenerateAccessToken() error = %v", err)
	}
...
func TestDispatchRiderAuthRequiredRejectsRefreshTokenAsAccessToken(t *testing.T) {
	tokens := authusecases.NewTokenUsecase([]byte("middleware-secret"), 15*time.Minute, 30*24*time.Hour)
	rawToken, _, err := tokens.GenerateRefreshToken(uuid.NewString(), "+2348012345678", uuid.NewString())
	if err != nil {
