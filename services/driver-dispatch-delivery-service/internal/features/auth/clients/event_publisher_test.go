...
func TestAuthEventTopicConstants(t *testing.T) {
	cases := map[string]string{
		...
		"phone changed":     TopicPhoneChanged,
	...
	want := map[string]string{
		...
		"phone changed":     "provider.phone_changed",
...
func TestPhoneChangedEventPayloadContract(t *testing.T) {
	changedAt := time.Date(2026, 6, 2, 10, 0, 0, 0, time.UTC)
	event := PhoneChangedEvent{
		Event:         TopicPhoneChanged,
		CorrelationID: "req-phone",
		ProviderID:    "provider-1",
		OldPhone:      "+2348012345678",
	...
	for _, field := range []string{"event", "correlation_id", "provider_id", "old_phone", "new_phone", "changed_at"} {
		if decoded[field] == "" {
			t.Fatalf("field %q must be present", field)
		}
	}
	if decoded["event"] != TopicPhoneChanged {
		t.Fatalf("event = %v, want %s", decoded["event"], TopicPhoneChanged)
	}
}
