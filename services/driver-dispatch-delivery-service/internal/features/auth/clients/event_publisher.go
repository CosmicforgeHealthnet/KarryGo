...
// TopicPhoneChanged is published after a dispatch rider successfully changes their phone number.
const TopicPhoneChanged = "provider.phone_changed"
...
// PhoneChangedEvent is published after a dispatch rider successfully updates their phone number.
...
type EventPublisher interface {
	...
	PublishPhoneChanged(ctx context.Context, event PhoneChangedEvent) error
}
...
// PublishPhoneChanged serialises the event and publishes it to TopicPhoneChanged.
func (p *RedisEventPublisher) PublishPhoneChanged(ctx context.Context, event PhoneChangedEvent) error {
	payload, err := json.Marshal(event)
	if err != nil {
		return fmt.Errorf("marshal phone_changed event: %w", err)
	}
	return p.client.Publish(ctx, TopicPhoneChanged, payload).Err()
}
