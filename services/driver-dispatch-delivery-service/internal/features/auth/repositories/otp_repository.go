...
type OTPRepository interface {
	...
	LatestByPhoneAndPurpose(ctx context.Context, phoneNumber, purpose string) (authmodels.OTP, bool, error)
...
func (r *PostgresOTPRepository) LatestByPhoneAndPurpose(ctx context.Context, phoneNumber, purpose string) (authmodels.OTP, bool, error) {
	row := r.db.QueryRow(ctx, `
		SELECT id::text, phone_number, email, purpose, otp_code_hash, attempts, max_attempts, expires_at, verified, locked_until, created_at, updated_at
		FROM dispatch_rider_otps
		WHERE phone_number = $1
