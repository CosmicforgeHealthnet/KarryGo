...
type SessionRepository interface {
	...
	RevokeAllByDispatchRiderID(ctx context.Context, dispatchRiderID string) (int64, error)
}
...
func (r *PostgresSessionRepository) RevokeAllByDispatchRiderID(ctx context.Context, dispatchRiderID string) (int64, error) {
	tag, err := r.db.Exec(ctx, `
		UPDATE dispatch_rider_sessions
		SET revoked_at = now(),
			updated_at = now()
		WHERE dispatch_rider_id = $1
		  AND revoked_at IS NULL
	`, dispatchRiderID)
	if err != nil {
		return 0, err
