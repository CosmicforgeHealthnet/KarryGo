...
const tripColumns = `
	...
	package_desc, package_weight::float8, started_at, arrived_at, completed_at,
...
func scanTrip(row rowScanner) (*Trip, error) {
	...
	if err := row.Scan(
		...
		&result.ReceiverPhone, &packageDesc, &packageWeight, &startedAt, &arrivedAt,
		&completedAt, &cancelledAt, &result.CreatedAt, &result.UpdatedAt,
	); err != nil {
		return nil, err
	}
