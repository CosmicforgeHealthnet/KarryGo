...
// ── fakeRepository: customer rating stubs (GAP 4) ───────────────────────────

func TestRateCustomerCompletedTripPublishesEvent(t *testing.T) {
	repo, pub, svc := newCustomerCancelEnv(t)
	providerID := uuid.NewString()
	tr := seedTripByBookingID(repo, providerID, uuid.NewString(), StatusCompleted)
	comment := "Good customer"

	rating, err := svc.RateCustomer(context.Background(), tr.ID, providerID, RateCustomerInput{
		Score:   5,
		Comment: &comment,
	})
	if err != nil {
		t.Fatalf("RateCustomer() error = %v", err)
	}
	if rating.TripID != tr.ID || rating.ProviderID != providerID || rating.CustomerID != tr.CustomerID || rating.Score != 5 {
		t.Fatalf("rating mismatch: %+v", rating)
	}
	if len(pub.customerRated) != 1 {
		t.Fatalf("customerRated events = %d, want 1", len(pub.customerRated))
	}
	event := pub.customerRated[0]
	if event.Event != TopicCustomerRated || event.RatingID != rating.ID || event.TripID != tr.ID || event.ProviderID != providerID || event.CustomerID != tr.CustomerID || event.Score != 5 {
		t.Fatalf("event mismatch: %+v", event)
	}
	if event.Comment == nil || *event.Comment != comment {
		t.Fatalf("event comment mismatch: %+v", event.Comment)
	}
	if event.CreatedAt.IsZero() || event.OccurredAt.IsZero() {
		t.Fatalf("event timestamps missing: %+v", event)
	}
}

func TestRateCustomerRejectsNonOwnedTripAsNotFound(t *testing.T) {
	repo, _, svc := newCustomerCancelEnv(t)
	tr := seedTripByBookingID(repo, uuid.NewString(), uuid.NewString(), StatusCompleted)

	_, err := svc.RateCustomer(context.Background(), tr.ID, uuid.NewString(), RateCustomerInput{Score: 5})
	assertAppErrorCode(t, err, apperrors.CodeNotFound)
}

func TestRateCustomerRejectsNonCompletedTrip(t *testing.T) {
	repo, _, svc := newCustomerCancelEnv(t)
	providerID := uuid.NewString()
	tr := seedTripByBookingID(repo, providerID, uuid.NewString(), StatusInProgress)

	_, err := svc.RateCustomer(context.Background(), tr.ID, providerID, RateCustomerInput{Score: 5})
	assertAppErrorCode(t, err, apperrors.CodeConflict)
}

func TestRateCustomerRejectsDuplicateTripRating(t *testing.T) {
	repo, _, svc := newCustomerCancelEnv(t)
	providerID := uuid.NewString()
	tr := seedTripByBookingID(repo, providerID, uuid.NewString(), StatusCompleted)

	if _, err := svc.RateCustomer(context.Background(), tr.ID, providerID, RateCustomerInput{Score: 5}); err != nil {
		t.Fatalf("first RateCustomer() error = %v", err)
	}
	_, err := svc.RateCustomer(context.Background(), tr.ID, providerID, RateCustomerInput{Score: 4})
	assertAppErrorCode(t, err, apperrors.CodeConflict)
}

func (r *fakeRepository) InsertCustomerRating(_ context.Context, tripID, providerID, customerID string, score int, comment *string) (CustomerRating, bool, error) {
	if r.customerRatings == nil {
		r.customerRatings = make(map[string]CustomerRating)
	}
	if _, exists := r.customerRatings[tripID]; exists {
		return CustomerRating{}, false, nil
	}
	rating := CustomerRating{ID: uuid.NewString(), TripID: tripID, ProviderID: providerID, CustomerID: customerID, Score: score, Comment: comment, CreatedAt: time.Now()}
	r.customerRatings[tripID] = rating
	return rating, true, nil
}

func (r *fakeRepository) GetCustomerRatingByTripID(_ context.Context, tripID string) (CustomerRating, bool, error) {
	rating, ok := r.customerRatings[tripID]
	return rating, ok, nil
}

// ── fakeTripEventPublisher: PublishCustomerRated stub (GAP 4) ────────────────

func (f *fakeTripEventPublisher) PublishCustomerRated(_ context.Context, event CustomerRatedEvent) error {
	f.customerRated = append(f.customerRated, event)
	return nil
}
...
func assertAppErrorCode(t *testing.T, err error, want apperrors.Code) {
	...
	if !ok {
		t.Fatalf("error = %T %v, want *apperrors.Error code %s", err, err, want)
	}
	if appErr.Code != want {
