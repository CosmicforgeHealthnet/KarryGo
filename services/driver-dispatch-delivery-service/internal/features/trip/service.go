...
// TripEventPublisher publishes outbound trip lifecycle events (Phase 7F–7N).
...
func (s *Service) HandleRequestAccepted(ctx context.Context, event RequestAcceptedEvent) (*Trip, error) {
	...
	if s.events != nil {
		_ = s.events.PublishTripCreated(ctx, TripCreatedEvent{
			...
			Status: string(trip.Status), CreatedAt: trip.CreatedAt, OccurredAt: s.now(),
		})
	}
	return trip, nil
}
...
// MarkArrived transitions the trip from assigned/en_route_pickup to arrived_pickup (Phase 7F).
...
// StartTrip transitions the trip from arrived_pickup to in_progress (Phase 7G).
...
// SubmitProof validates, saves, and records proof of delivery for an in_progress trip (Phase 7H).
...
// CompleteTrip transitions the trip from proof_submitted to completed (Phase 7J).
...
// CancelTrip cancels a provider-owned trip, applies penalty/suspension rules, and publishes events (Phase 7K).
func (s *Service) CancelTrip(ctx context.Context, tripID, providerID string, req CancelRequest) (CancelResponse, error) {
	...
	updatedTrip, _, err := s.repository.CancelTripTx(ctx, CancelTripInput{
		TripID: tripID, ProviderID: providerID, FromStatus: trip.Status,
