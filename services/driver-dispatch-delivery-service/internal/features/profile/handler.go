...
const maxAvatarMemory = 5 << 20 // 5 MB multipart buffer
...
func NewHandlerWithUploader(service ProviderService, uploader AvatarUploader) *Handler {
...
func RegisterRoutes(engine *gin.Engine, tokens *authusecases.TokenUsecase, handler *Handler) {
	...
	protected.PATCH("/settings", handler.UpdateSettings)
	protected.POST("/me/avatar", handler.UploadAvatar)
	protected.DELETE("/me", handler.DeleteMe)
}
...
func (h *Handler) Onboarding(c *gin.Context) {
	...
	respondOK(c, gin.H{"profile_photo_url": result.ProfilePhotoURL})
}
...
func (h *Handler) GetSettings(c *gin.Context) {
	result, err := h.service.GetSettings(c.Request.Context(), authFromContext(c))
	if err != nil {
		httpx.Abort(c, err)
		return
	}
...
func (h *Handler) UpdateSettings(c *gin.Context) {
	...
	if err := decodeStrictProfileJSON(c, &req, settingsFields()); err != nil {
		httpx.Abort(c, err)
		return
	}
	result, err := h.service.UpdateSettings(c.Request.Context(), authFromContext(c), req)
	if err != nil {
		httpx.Abort(c, err)
		return
	}
...
func (h *Handler) DeleteMe(c *gin.Context) {
	if err := h.service.DeleteMe(c.Request.Context(), authFromContext(c)); err != nil {
		httpx.Abort(c, err)
		return
	}
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"data":    gin.H{"message": "Account has been deactivated successfully."},
	})
}

func (h *Handler) UploadAvatar(c *gin.Context) {
	if h.uploader == nil {
		httpx.Abort(c, apperrors.Internal("Avatar storage is not configured.", nil))
		return
	}
	if err := c.Request.ParseMultipartForm(maxAvatarMemory); err != nil {
		httpx.Abort(c, apperrors.Validation("Check your details.", []apperrors.FieldViolation{
			{Field: "avatar", Message: "Invalid multipart form."},
		}))
		return
	}
	...
	result, err := h.service.UploadAvatar(
		c.Request.Context(),
		authFromContext(c),
		h.uploader,
		file,
...
func decodeStrictProfileJSON(c *gin.Context, out any, allowed map[string]struct{}) error {
	body, err := io.ReadAll(c.Request.Body)
	if err != nil {
		return validationBadRequest("Check your details.", []apperrors.FieldViolation{
			{Field: "body", Message: "Invalid request body."},
...
func settingsFields() map[string]struct{} {
