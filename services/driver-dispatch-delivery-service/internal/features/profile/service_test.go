...
func TestSettingsUpdateSupportsRequestedFields(t *testing.T) {
	repo := newFakeProfileRepository()
	repo.mustEnsure("provider-123", "+2348000000001")
	service := NewService(repo)
	...
	dark := true
	language := "yo"
	settings, err := service.UpdateSettings(context.Background(), testAuth(), UpdateSettingsInput{
		PushEnabled:     &push,
		SMSEnabled:      &sms,
		Language:        &language,
	...
	if settings.PushEnabled || settings.SMSEnabled || settings.Language != "yo" || !settings.DarkModeEnabled {
		t.Fatalf("settings mismatch: %+v", settings)
	}
}

func TestSettingsRejectsInvalidLanguage(t *testing.T) {
	repo := newFakeProfileRepository()
	repo.mustEnsure("provider-123", "+2348000000001")
	service := NewService(repo)
	language := "xx"

	_, err := service.UpdateSettings(context.Background(), testAuth(), UpdateSettingsInput{Language: &language})
	assertValidationField(t, err, "language")
}

func TestUploadAvatarAcceptsPNGAndUpdatesProfilePhotoURL(t *testing.T) {
	repo := newFakeProfileRepository()
	repo.mustEnsure("provider-123", "+2348000000001")
	uploader := &fakeAvatarUploader{}
	service := NewServiceWithEvents(repo, &fakeProfileEventPublisher{})
	png := testPNGBytes()

	result, err := service.UploadAvatar(
		context.Background(),
		testAuth(),
		uploader,
		bytes.NewReader(png),
	...
	if result.ProfilePhotoURL == nil || *result.ProfilePhotoURL == "" {
		t.Fatalf("profile_photo_url was not updated: %+v", result)
	}
	if uploader.objectPath == "" || !bytes.Contains([]byte(uploader.objectPath), []byte("profiles/provider-123/avatar_")) {
		t.Fatalf("unexpected object path: %q", uploader.objectPath)
	}
	if bytes.Contains([]byte(*result.ProfilePhotoURL), []byte("\\")) {
		t.Fatalf("profile_photo_url exposes filesystem path: %q", *result.ProfilePhotoURL)
	}
}

func TestUploadAvatarRejectsDeclaredMimeMismatch(t *testing.T) {
	repo := newFakeProfileRepository()
	repo.mustEnsure("provider-123", "+2348000000001")
	service := NewService(repo)
	png := testPNGBytes()

	_, err := service.UploadAvatar(
		context.Background(),
		testAuth(),
		&fakeAvatarUploader{},
		bytes.NewReader(png),
	...
	assertValidationField(t, err, "avatar")
}
...
func (f *fakeAvatarUploader) Upload(_ context.Context, objectPath string, file io.Reader) (string, error) {
	f.objectPath = objectPath
	if _, err := io.ReadAll(file); err != nil {
		return "", err
	}
	return "local-private://" + objectPath, nil
}
...
func (r *fakeProfileRepository) GetOrCreateSettings(_ context.Context, providerID string) (ProviderSettings, error) {
	return ProviderSettings{ProviderID: providerID, PushEnabled: true, SMSEnabled: true, Language: "en"}, nil
}

func (r *fakeProfileRepository) UpdateSettings(_ context.Context, providerID string, input UpdateSettingsInput) (ProviderSettings, error) {
	s := ProviderSettings{ProviderID: providerID, PushEnabled: true, SMSEnabled: true, Language: "en"}
	if input.PushEnabled != nil {
		s.PushEnabled = *input.PushEnabled
	}
	if input.SMSEnabled != nil {
		s.SMSEnabled = *input.SMSEnabled
	}
	if input.Language != nil {
		s.Language = *input.Language
	}
	if input.DarkModeEnabled != nil {
		s.DarkModeEnabled = *input.DarkModeEnabled
	}
	return s, nil
}

func (r *fakeProfileRepository) UpdateProfilePhotoURL(_ context.Context, providerID, url string) error {
	if p, ok := r.providers[providerID]; ok {
		p.ProfilePhotoURL = &url
		r.providers[providerID] = p
	}
	return nil
}

func (r *fakeProfileRepository) DeactivateProvider(_ context.Context, providerID string) error {
	if p, ok := r.providers[providerID]; ok {
		p.IsActive = false
		r.providers[providerID] = p
		return nil
