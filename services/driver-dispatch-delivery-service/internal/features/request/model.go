...
func NewProviderRequestInboxItem(inbox ProviderRequestInbox, now time.Time) (ProviderRequestInboxItem, bool, error) {
	...
	return ProviderRequestInboxItem{
		...
		DropoffAddress: booking.DropoffAddress, PackageDesc: booking.PackageDesc,
...
type RequestDetailResponse struct {
	...
	IsFragile        bool        `json:"is_fragile"`
	ServiceTier      string      `json:"service_tier"`
	ServiceTierLabel string      `json:"service_tier_label"`
	ReceiverName     string      `json:"receiver_name"`
	ReceiverPhone    string      `json:"receiver_phone"`
	RemainingSeconds int64       `json:"remaining_seconds"`
	ExpiresAt        time.Time   `json:"expires_at"`
...
func serviceTierLabel(tier string) string {
