...
func TestTripMigrationFilesExist(t *testing.T) {
	for _, name := range []string{
		...
		"000026_create_customer_ratings_by_provider.up.sql", "000026_create_customer_ratings_by_provider.down.sql",
	} {
...
func TestTripMigrationsApplyValidateAndRollback(t *testing.T) {
	...
	for _, name := range []string{
		...
		"000024_create_cancellations.up.sql", "000026_create_customer_ratings_by_provider.up.sql",
	} {
	...
	for _, index := range []string{
		...
		"idx_cancellations_cancelled_at", "idx_customer_ratings_trip_id", "idx_customer_ratings_provider_id",
		"idx_customer_ratings_customer_id",
	} {
	...
	if _, err := conn.Exec(ctx, `
		...
		VALUES ($1,$2,$3,5,'Good customer')
	`, tripID, providerID, customerID); err != nil {
		t.Fatal(err)
	}
	if _, err := conn.Exec(ctx, `
		INSERT INTO customer_ratings_by_provider (trip_id,provider_id,customer_id,score)
		VALUES ($1,$2,$3,4)
	`, tripID, providerID, customerID); err == nil {
		t.Fatal("duplicate customer rating accepted")
	}
	if _, err := conn.Exec(ctx, `
		INSERT INTO customer_ratings_by_provider (trip_id,provider_id,customer_id,score)
		VALUES ($1,$2,$3,6)
	`, uuid.NewString(), providerID, customerID); err == nil {
		t.Fatal("invalid customer rating score accepted")
	}

	for _, name := range []string{
		"000026_create_customer_ratings_by_provider.down.sql", "000024_create_cancellations.down.sql", "000023_create_delivery_proofs.down.sql",
		"000022_create_trip_state_log.down.sql", "000021_create_trips.down.sql",
	} {
		execMigrationFile(t, ctx, conn, name)
	}
	for _, table := range []string{"customer_ratings_by_provider", "cancellations", "delivery_proofs", "trip_state_log", "trips"} {
