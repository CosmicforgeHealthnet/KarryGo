  ...
  void _submit() async {
    ...
    await guarantorRes.when(
      success: (_) async {
        ...
        final emergencyRes = await widget.profileController
    ...
    if (hasError) {
      setState(() => _isLoading = false);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(errMsg), backgroundColor: Colors.red.shade800),
      );
    } else {
      try {
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const _FieldLabel(
                      label: 'Upload Vehicle Registration Sticker',
                    ),
                    const SizedBox(height: 8),
                    DocumentUploadBox(
                    ...
                    const _InfoBanner(
                      text:
                          'Provide emergency contact information. This could be a family member or close individual to guarantor.',
                    ),
                    const SizedBox(height: 20),
                    const Text(
                      'Emergency Contact',
