...
class _DispatchProviderAppState extends State<DispatchProviderApp> {
  ...
  void initState() {
    ...
    _authController = DispatchAuthController(
      api: DispatchAuthApi(
        ApiCoreConfig(baseUrl: AppConfig.apiBaseUrl),
      ),
      tokenStorage: _tokenStorage,
    );
    _profileController = ProviderProfileController(
      api: ProviderProfileApi(
        ApiCoreConfig(baseUrl: AppConfig.apiBaseUrl),
      ),
  ...
  Widget _buildScreen() {
    return switch (_screen) {
      ...
      _DispatchScreen.profileDetails => ProfileDetailsScreen(
          initialPhone: _authController.currentPhoneNumber ?? _phone,
      ...
      _DispatchScreen.photoUpload => PhotoUploadScreen(
          onContinue: (selfiePath) async {
            ...
            debugPrint('[PHOTO_UPLOAD] Selfie selected: $selfiePath — storing locally.');
          ...
          onSubmit: (data) async {
            ...
            if (_bikeTypeData != null) {
              ...
              vehicleRes.when(
                ...
                failure: (err) {
                  debugPrint('[SUBMIT] Vehicle failed (H): ${err.message} fields=${err.fields.map((f) => f.message).join(', ')}');
              ...
              if (vehicleId != null &&
                ...
                debugPrint('[SUBMIT] Uploading vehicle doc (I): vehicleId=$vehicleId');
            ...
            debugPrint('[SUBMIT] All steps complete — navigating to verificationPending.');
  ...
  Future<void> _checkSavedSession() async {
    ...
    final saved = await _tokenStorage.readTokens();

    if (!saved.hasAny || saved.accessToken == null) {
    ...
    profileResult.when(
      ...
      failure: (error) {
        debugPrint('[AUTH] Profile load failed: ${error.message}');
    ...
    if (isUnauthorized && saved.refreshToken != null) {
      ...
      if (refreshed && newAccess != null) {
        ...
        retryResult.when(
          ...
          failure: (error) {
            debugPrint('[AUTH] Profile load after refresh failed: ${error.message}');
          },
        );
    ...
    // All attempts failed — clear stored tokens and show Login.
    debugPrint('[AUTH] Session restore failed → clearing tokens → Login');
    try {
  ...
  Future<void> _handlePostVerify() async {
    debugPrint('[AUTH] _handlePostVerify | isLoginMode=$_isLoginMode');

    if (!_isLoginMode) {
      debugPrint('[AUTH] Signup flow → AccountType');
      _go(_DispatchScreen.accountType);
      return;
    }

    debugPrint('[AUTH] Login flow → calling GET /api/v1/provider/me');
    ...
    result.when(
      success: (profile) {
        debugPrint(
          '[AUTH] /provider/me success | onboardingComplete=${profile.onboardingComplete}',
        );
        if (profile.onboardingComplete) {
          _go(_DispatchScreen.home);
        } else {
      ...
      failure: (error) {
        debugPrint('[AUTH] /provider/me failed (${error.message}) → AccountType (safe fallback)');
        _go(_DispatchScreen.accountType);
      },
    );
  }
  ...
  Future<bool> _waitForVerificationSteps() async {
    ...
    for (int attempt = 0; attempt < maxAttempts; attempt++) {
      if (attempt > 0) {
        debugPrint('[VERIFICATION] Steps not ready — retrying in ${delay.inMilliseconds}ms (attempt ${attempt + 1}/$maxAttempts)');
        await Future.delayed(delay);
      }
      debugPrint('[VERIFICATION] Polling GET /verification/status (attempt ${attempt + 1}/$maxAttempts)');
      final result = await _verificationController.loadVerificationStatus();
      bool ready = false;
      result.when(
        success: (status) {
          debugPrint('[VERIFICATION] Status: overall=${status.overallStatus}, steps=${status.steps.length}');
          ready = status.steps.isNotEmpty;
        },
        failure: (err) {
          // 404 = steps not yet initialized — retry silently
          debugPrint('[VERIFICATION] Status poll failed: ${err.message} (code=${err.code})');
        },
      );
      if (ready) return true;
    }
    debugPrint('[VERIFICATION] Steps still not initialized after $maxAttempts attempts');
    return false;
  }
